    ...
    if oyuncu then
        local function islem_tamam(oyuncu)  
            ...
            if oyuncu.Functions.RemoveMoney("bank", ucret) then
                ...
                TriggerEvent('DiscordBot:ToDiscord', 'isletmesatinal', "İsimli Oyuncu ``"..tablo.veri.job.."`` Adlı İşletmeyi ``"..tablo.veri.ucret.."$`` Fiyatıyla ``"..tablo.deger.."lık`` Kiraladı" , oyuncu.PlayerData.source)
                cb(true)
            else
                cb(false,{
